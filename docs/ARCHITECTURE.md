...
Trust boundaries: the authorized demo signal operator emits on Sepolia -> proof builder -> Gateway/precompile -> Overseer -> Bulkhead. The signal contract is a controlled demo source, not an economic oracle. Gateway verification does not by itself imply source transaction success; receipt status is checked independently.
...
- Factory: only its operator can create units; each cluster is capped at seven and must be finalized before Overseer registration; every deployment gets a fresh address and immutable cluster id.
...
- Frontend: live mode is read-only and derives Bulkhead state from Creditcoin RPC plus `BulkheadHalted`; preview mode is explicitly fixture-only.
